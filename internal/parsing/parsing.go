...
type AbstractParserWithWordBoundary struct {
	...
	innerExtract func(context *kronos.InternalParsingContext, match []string) any
...
func (p *AbstractParserWithWordBoundary) Extract(context *kronos.InternalParsingContext, match []string) any {
	if len(match) < 2 {
