...
func TestTimezoneExpression_UTCOffset(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.expectedMinute, *result.Start().Get(kronos.ComponentMinute))
...
func TestTimezoneExpression_DateRange(t *testing.T) {
	...
	assert.Equal(t, 1, *result.Start().Get(kronos.ComponentDay))
	assert.Equal(t, 10, *result.Start().Get(kronos.ComponentHour))
	assert.Equal(t, 30, *result.Start().Get(kronos.ComponentMinute))
	assert.Equal(t, 9*60, *result.Start().Get(kronos.ComponentTimezoneOffset)) // JST: GMT+9:00

	// End date
	assert.NotNil(t, result.End())
	assert.Equal(t, 2016, *result.End().Get(kronos.ComponentYear))
	assert.Equal(t, 10, *result.End().Get(kronos.ComponentMonth))
	assert.Equal(t, 2, *result.End().Get(kronos.ComponentDay))
	assert.Equal(t, 10, *result.End().Get(kronos.ComponentHour))
	assert.Equal(t, 30, *result.End().Get(kronos.ComponentMinute))
	assert.Equal(t, -8*60, *result.End().Get(kronos.ComponentTimezoneOffset)) // PST: UTC−08:00
}
...
func TestTimezoneExpression_AmbiguousET(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, 23, *result.Start().Get(kronos.ComponentHour))
			assert.Equal(t, tt.expectedTimezoneOffset, *result.Start().Get(kronos.ComponentTimezoneOffset))
		})
	}
}
...
func TestTimezoneExpression_CustomTimezone(t *testing.T) {
	...
	t.Run("XYZ not recognized without custom timezone", func(t *testing.T) {
		...
		assert.Equal(t, expectedDate.Year(), *result.Start().Get(kronos.ComponentYear))
		assert.Equal(t, int(expectedDate.Month()), *result.Start().Get(kronos.ComponentMonth))
		assert.Equal(t, expectedDate.Day(), *result.Start().Get(kronos.ComponentDay))
		assert.Equal(t, expectedDate.Hour(), *result.Start().Get(kronos.ComponentHour))
	})
	...
	t.Run("XYZ recognized with custom fixed timezone", func(t *testing.T) {
		...
		assert.Equal(t, -180, *result.Start().Get(kronos.ComponentTimezoneOffset))
	})
	...
	t.Run("XYZ with ambiguous timezone non-DST date", func(t *testing.T) {
		...
		assert.Equal(t, "Jan 1st 2023 at 10:00 XYZ", result.Text())
		assert.Equal(t, -180, *result.Start().Get(kronos.ComponentTimezoneOffset)) // Non-DST
	})
	...
	t.Run("XYZ with ambiguous timezone DST date", func(t *testing.T) {
		...
		assert.Equal(t, -120, *result.Start().Get(kronos.ComponentTimezoneOffset)) // DST
	})
}
...
func TestTimezoneExpression_DateWithTimezone(t *testing.T) {
	...
	assert.Equal(t, -300, *result.Start().Get(kronos.ComponentTimezoneOffset))
}
...
func TestTimezoneExpression_NotParsingFromRelativeTime(t *testing.T) {
	...
	t.Run("in 1 hour get eggs and milk", func(t *testing.T) {
		...
		assert.Equal(t, expectedOffset, *result.Start().Get(kronos.ComponentTimezoneOffset))
