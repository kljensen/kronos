...
func TestSlashDateFormatParser(t *testing.T) {
	...
	t.Run("MM/DD/YYYY format", func(t *testing.T) {
		...
		parsedResult := result.(*kronos.ParsingResult)

		assert.Equal(t, 2012, *parsedResult.Start().Get(kronos.ComponentYear))
		assert.Equal(t, 8, *parsedResult.Start().Get(kronos.ComponentMonth))
		assert.Equal(t, 10, *parsedResult.Start().Get(kronos.ComponentDay))

		tags := parsedResult.Tags()
		assert.True(t, tags["parser/SlashDateFormatParser"])
	})
	...
	t.Run("DD/MM/YYYY format (little endian)", func(t *testing.T) {
		...
		assert.Equal(t, 2012, *parsedResult.Start().Get(kronos.ComponentYear))
		assert.Equal(t, 10, *parsedResult.Start().Get(kronos.ComponentMonth))
		assert.Equal(t, 8, *parsedResult.Start().Get(kronos.ComponentDay))
	})
	...
	t.Run("auto-swap invalid month", func(t *testing.T) {
		...
		assert.Equal(t, 8, *parsedResult.Start().Get(kronos.ComponentMonth))
		assert.Equal(t, 15, *parsedResult.Start().Get(kronos.ComponentDay))
	})
	...
	t.Run("MM/DD without year", func(t *testing.T) {
		...
		assert.False(t, parsedResult.Start().IsCertain(kronos.ComponentYear))
		assert.NotNil(t, parsedResult.Start().Get(kronos.ComponentYear))
	})
	...
	t.Run("2-digit year", func(t *testing.T) {
		...
		assert.Equal(t, 2016, *parsedResult.Start().Get(kronos.ComponentYear))
		assert.Equal(t, 12, *parsedResult.Start().Get(kronos.ComponentMonth))
		assert.Equal(t, 30, *parsedResult.Start().Get(kronos.ComponentDay))
	})
	...
	t.Run("with dot separator and year", func(t *testing.T) {
		...
		assert.Equal(t, 2020, *parsedResult.Start().Get(kronos.ComponentYear))
	})
...
func TestSlashDateFormatParserIntegration(t *testing.T) {
	...
	t.Run("full parse with slash", func(t *testing.T) {
		...
		parsedResult := result.(*kronos.ParsingResult)
		assert.Equal(t, 4, *parsedResult.Start().Get(kronos.ComponentMonth))
		assert.Equal(t, 15, *parsedResult.Start().Get(kronos.ComponentDay))
		assert.Equal(t, 2016, *parsedResult.Start().Get(kronos.ComponentYear))
	})
}
