...
func TestToTimezoneOffset(t *testing.T) {
	instant := time.Date(2024, 11, 2, 14, 30, 0, 0, time.UTC)

	t.Run("toTimezoneOffset with int returns value", func(t *testing.T) {
		offset := toTimezoneOffset(-300, instant, nil)

		if offset != -300 {
			t.Errorf("Expected offset -300, got %d", offset)
		}
	})

	t.Run("toTimezoneOffset with nil returns 0", func(t *testing.T) {
		offset := toTimezoneOffset(nil, instant, nil)
	...
	t.Run("toTimezoneOffset with timezone name in overrides", func(t *testing.T) {
		overrides := make(TimezoneAbbrMap)
		overrides["EST"] = -300

		offset := toTimezoneOffset("EST", instant, overrides)
	...
	t.Run("toTimezoneOffset with ambiguous timezone during DST", func(t *testing.T) {
		overrides := make(TimezoneAbbrMap)
		overrides["ET"] = AmbiguousTimezoneMap{
		...
		// During DST (summer)
		summerInstant := time.Date(2024, 7, 1, 12, 0, 0, 0, time.UTC)
		offset := toTimezoneOffset("ET", summerInstant, overrides)

		if offset != -240 {
			t.Errorf("Expected DST offset -240, got %d", offset)
		}

		// Outside DST (winter)
		winterInstant := time.Date(2024, 1, 1, 12, 0, 0, 0, time.UTC)
		offset = toTimezoneOffset("ET", winterInstant, overrides)

		if offset != -300 {
			t.Errorf("Expected non-DST offset -300, got %d", offset)
		}
	})

	t.Run("toTimezoneOffset with location name", func(t *testing.T) {
		offset := toTimezoneOffset("America/New_York", instant, nil)

		// The offset will vary based on DST, but it should be either -240 or -300
		if offset != -240 && offset != -300 {
			t.Logf("Offset for America/New_York: %d (expected -240 or -300)", offset)
		}
	})
}
